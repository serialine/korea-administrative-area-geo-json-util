...
export const unionPolygons = (polygons) => {
    ...
    reduce((acc, value) => (acc === undefined ? value : union(acc, value)), undefined), 
    // 병합된 위경도 배열을 MultiPolygon Feature혹은 Polygon Feature로 매핑
    map((unionedCoordinates) => (is3DPosition(unionedCoordinates)
        ? multiPolygon(unionedCoordinates)
        : polygon(unionedCoordinates))));
};
