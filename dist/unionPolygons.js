 ...
 */
const union = (coordinates1, coordinates2) => martinez.union(coordinates1, coordinates2);
/**
...
const unionPolygons = (polygons) => {
    ...
    // 폴리곤 => 위경도 배열로 매핑
    operators_1.map((_polygon) => _polygon.geometry.coordinates), 
    // 1ms씩 딜레이를 주어 전달 (스레드를 너무 오래 블락하지 않도록)
    operators_1.concatMap((value) => rxjs_1.of(value).pipe(operators_1.delay(1))), 
    // 위경도 배열의 배열을 순회하며 하나로 병합
    operators_1.reduce((acc, value) => (acc === undefined ? value : union(acc, value)), undefined), 
    // 병합된 위경도 배열을 MultiPolygon Feature혹은 Polygon Feature로 매핑
    operators_1.map((unionedCoordinates) => (GeoUtils_1.is3DPosition(unionedCoordinates)
