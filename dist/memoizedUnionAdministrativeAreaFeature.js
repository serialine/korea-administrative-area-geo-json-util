...
const unionPolygons_1 = require("./unionPolygons");
/**
 * 행정구역 geojson 배열을 key로 변환합니다.
 */
const getCacheKey = (features) => features.map((value) => value.properties.adm_cd);
/**
 ...
 * @param features
 */
const memoizedUnionAdministrativeAreaFeature = (features, cache) => {
    return rxjs_1.defer(() => {
        ...
        if (largestSubKey !== undefined) {
            ...
            const remainingKey = cacheKey.filter((value) => !largestSubKey.includes(value));
            ...
            exports.memoizedUnionAdministrativeAreaFeature(features.filter((feature) => remainingKey.includes(feature.properties.adm_cd)), cache) // 나머지 부분 폴리곤 병합 결과
            ).pipe(operators_1.map((value) => value.filter(validation_1.isNotNil)), operators_1.concatMap((value) => unionPolygons_1.unionPolygons(value)), // 기존 캐시된 폴리곤과 나머지 부분 폴리곤을 병합
            operators_1.tap((result) => result && cache.set(cacheKey, result)) // 병합결과를 캐시에 저장합니다.
            );
        }
        // 전혀 캐시되어있지 않은 경우
        if (features.length > 2) {
            ...
            ).pipe(operators_1.map((value) => value.filter(validation_1.isNotNil)), operators_1.concatMap((value) => unionPolygons_1.unionPolygons(value)), // 각 결과를 병합
            operators_1.tap((result) => result && cache.set(cacheKey, result)) // 병합 결과를 캐시에 저장
            );
        }
        // 아이템이 2개 이하인 경우, 병합하고 캐시에 저장하고 반환합니다.
        return unionPolygons_1.unionPolygons(features).pipe(operators_1.tap((result) => result && cache.set(cacheKey, result)));
    });
};
