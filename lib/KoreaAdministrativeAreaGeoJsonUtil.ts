...
export class KoreaAdministrativeAreaGeoJsonUtil {
   ...
   */
  getKorea() {
    return of(this.admAllGeoJson);
  }

  /**
   * 주어진 행정구역 목록을 병합한 GeoJSON 조회
   */
  getByAdministrativeArea(
    administrativeAreaCodes: string[]
  ): Observable<FeatureCollection<Polygon | MultiPolygon> | null> {
    // 행정구역 목록이 비어있을경우 null 반환
    if (administrativeAreaCodes.length === 0) return of(null);

    return of(administrativeAreaCodes).pipe(
